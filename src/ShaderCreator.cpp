...
void ShaderCreator::instance(const ptree::value_type & v)
{
    ...
    if (type == "Mirror")
    {
        e = new BlinnPhongShader(name, type, diffuse, specular, phongExp, 1.0f);
        scene->add(e);
    ...
    buf >> diffuse.x; buf >> diffuse.y; buf >> diffuse.z;
    buf.clear();
    ...
    {
        buf.str(*pSpecular);
        buf >> specular.x; buf >> specular.y; buf >> specular.z;
        buf.clear();
    }
    if (pPhongExp)
    {
    ...
    {
        e = new LambertianShader(name, type, diffuse, mirrorCoef);
    }
    else if (type == "BlinnPhong" || type == "Phong" || type == "BlinnPhongMirrored")
    {
        if (hasMirror)
        {
            e = new BlinnPhongShader(name, type, diffuse, specular, phongExp, mirrorCoef, roughness);
        }
        else
        {
            e = new BlinnPhongShader(name, type, diffuse, specular, phongExp);
        }
    }
    scene->add(e);
    std::cout << "finished." << std::endl;
