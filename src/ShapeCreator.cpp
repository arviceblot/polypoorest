...
void ShapeCreator::instance(const ptree::value_type& v)
{
    ...
    auto shaderRef = scene->getShader(shaderName);

    std::shared_ptr<Shape> e = NULL;
    ...
    if (type == "sphere")
    {
        ...
        e = std::make_shared<ImplicitSphere>(name, type, shaderRef, center, radius);
        scene->add(e);
    }
    else if (type == "triangle")
    {
        ...
        e = std::make_shared<Triangle>(name, type, shaderRef, v0, v1, v2);
        if (v.first == "instance")
        {
            scene->addBase(e);
        }
    ...
    else if (type == "box")
    {
        ...
        e = std::make_shared<Box>(name, type, shaderRef, minPt, maxPt);
        if (v.first == "instance")
        {
    ...
    else if (type == "instance")
    {
        ...
        auto shapeRef = scene->getInstance(id);
