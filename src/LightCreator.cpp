...
void LightCreator::instance(const ptree::value_type& v)
{
    ...
    buf >> position.x; buf >> position.y; buf >> position.z;
    buf.clear();

    buf.str(v.second.get<std::string>("intensity"));
    buf >> intensity.x; buf >> intensity.y; buf >> intensity.z;
    buf.clear();

    if (type == "point")
    {
        e = new PointLight(type, position, intensity);
    }
    else if (type == "area")
    {
        glm::vec3 normal;
        ...
        e = new AreaLight(type, position, intensity, normal, width, length);
    }
    scene->add(e);
